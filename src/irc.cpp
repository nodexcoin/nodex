...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #nodexcoinTEST\r");
            Send(hSocket, "WHO #nodexcoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #nodexcoin\n");
            // randomly join #nodexcoin00-#nodexcoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #nodexcoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #nodexcoin%02d\r", channel_number).c_str());
        };
