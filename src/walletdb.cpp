...
bool CWalletDB::WritenodexNodeConfig(std::string sAlias, const CnodexNodeConfig& nodeConfig)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("nodex"), sAlias), nodeConfig, true);
}

bool CWalletDB::ReadnodexNodeConfig(std::string sAlias, CnodexNodeConfig& nodeConfig)
{
    return Read(std::make_pair(std::string("nodex"), sAlias), nodeConfig);
}

bool CWalletDB::ErasenodexNodeConfig(std::string sAlias)
{
    nWalletDBUpdated++;
    return Erase(std::make_pair(std::string("nodex"), sAlias));
}
             ...
             CWalletScanState &wss, string& strType, string& strErr)
{
    try {
        ...
        else if (strType == "nodex")
	{
	    std::string sAlias;
	    ssKey >> sAlias;
	    CnodexNodeConfig nodexNodeConfig;
	    ssValue >> nodexNodeConfig;
	    pwallet->mapMynodexNodes.insert(make_pair(sAlias, nodexNodeConfig));
	}
    } catch (...)
    {
        return false;
