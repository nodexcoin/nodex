...
int nAnonymizenodexAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Mac: ~/Library/Application Support/nodex
    // Unix: ~/.nodex
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "nodex";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nodex.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nodexd.pid"));
