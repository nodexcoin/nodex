...
AddEditnodexNode::AddEditnodexNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditnodexNode)
{
    ui->setupUi(this);

}

AddEditnodexNode::~AddEditnodexNode()
{
    delete ui;
}


void AddEditnodexNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMynodexNodes.insert(make_pair(c.sAddress, c));
	walletdb.WritenodexNodeConfig(c.sAddress, c);
        uiInterface.NotifynodexNodeChanged(c);

        accept();
    }
}

void AddEditnodexNode::on_cancelButton_clicked()
{
    reject();
}
