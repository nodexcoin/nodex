...
static void NotifynodexNodeUpdated(MasternodeManager *page, CnodexNodeConfig nodeConfig)
{
    ...
    QMetaObject::invokeMethod(page, "updatenodexNode", Qt::QueuedConnection,
                              Q_ARG(QString, alias),
                              Q_ARG(QString, addr),
...
void MasternodeManager::subscribeToCoreSignals()
{
    ...
    uiInterface.NotifynodexNodeChanged.connect(boost::bind(&NotifynodexNodeUpdated, this, _1));
}

void MasternodeManager::unsubscribeFromCoreSignals()
{
    // Disconnect signals from core
    uiInterface.NotifynodexNodeChanged.disconnect(boost::bind(&NotifynodexNodeUpdated, this, _1));
}
...
void MasternodeManager::updatenodexNode(QString alias, QString addr, QString privkey, QString collateral)
{
    LOCK(cs_nodex);
    bool bFound = false;
    int nodeRow = 0;
...
void MasternodeManager::updateNodeList()
{
    ...
    {
        LOCK(cs_nodex);
        BOOST_FOREACH(PAIRTYPE(std::string, CnodexNodeConfig) nodex, pwalletMain->mapMynodexNodes)
        {
            updatenodexNode(QString::fromStdString(nodex.second.sAlias), QString::fromStdString(nodex.second.sAddress), QString::fromStdString(nodex.second.sMasternodePrivKey), QString::fromStdString(nodex.second.sCollateralAddress));
        }
    }
}
...
{
    AddEditnodexNode* aenode = new AddEditnodexNode();
    aenode->exec();
}
...
void MasternodeManager::on_getConfigButton_clicked()
{
    ...
    int r = index.row();
    std::string sAddress = ui->tableWidget_2->item(r, 1)->text().toStdString();
    CnodexNodeConfig c = pwalletMain->mapMynodexNodes[sAddress];
    std::string sPrivKey = c.sMasternodePrivKey;
    nodexNodeConfigDialog* d = new nodexNodeConfigDialog(this, QString::fromStdString(sAddress), QString::fromStdString(sPrivKey));
    d->exec();
}
...
void MasternodeManager::on_removeButton_clicked()
{
    ...
    confirm = QMessageBox::question(this, "Delete nodex Node?", "Are you sure you want to delete this nodex node configuration?", QMessageBox::Yes|QMessageBox::No);
    ...
    if(confirm == QMessageBox::Yes)
    {
        ...
        pwalletMain->mapMynodexNodes.erase(sAddress);
        walletdb.ErasenodexNodeConfig(c.sAddress);
...
void MasternodeManager::on_startButton_clicked()
{
    ...
    QMessageBox msg;
    if(result)
        msg.setText("nodex Node at " + QString::fromStdString(c.sAddress) + " started.");
    else
...
void MasternodeManager::on_stopButton_clicked()
{
    ...
    if(result)
    {
        msg.setText("nodex Node at " + QString::fromStdString(c.sAddress) + " stopped.");
    }
    else
    {
        msg.setText("Error: " + QString::fromStdString(errorMessage));
...
void MasternodeManager::on_startAllButton_clicked()
{
    ...
    BOOST_FOREACH(PAIRTYPE(std::string, CnodexNodeConfig) nodex, pwalletMain->mapMynodexNodes)
    {
        CnodexNodeConfig c = nodex.second;
	std::string errorMessage;
        bool result = activeMasternode.RegisterByPubKey(c.sAddress, c.sMasternodePrivKey, c.sCollateralAddress, errorMessage);
	if(result)
	{
