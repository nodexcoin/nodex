		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string nodexAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = nodexAddress;
				        if(richListData.mapRichList.find(nodexAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(nodexAddress, richListDataItem));
					
				        richListData.mapRichList[nodexAddress].nBalance = richListData.mapRichList[nodexAddress].nBalance - atxout.nValue;
