...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: nodex.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: nodexd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizenodexamount=<n> " + _("Keep N nodex anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. nodex is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. nodex is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "nodex server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of nodex") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart nodex to complete") << "\n";
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizenodexAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    /* Denominations
       ...
       For example:
       1NODEX+1000 == (.1NODEX+100)*10
       10NODEX+10000 == (1NODEX+1000)*10
    */
    ...
    */
	...
	{
	    uiInterface.NotifynodexNodeChanged(nodex.second);
	}
